...
public class LevelToolBar extends VBox implements TabInterface {
    ...
    private void createProperties() {
        ...
        myProperties.put("Number of troops to spawn", 3);
...
//		elementsToSpawn = imageList.stream().map(ImageView::getId).collect(Collectors.toList());
         ...
         */
        int level = Integer.valueOf(levelAndWave.split("\\.+")[0]);
        myController.setLevel(level);
        if (waveToData.containsKey(levelAndWave)) {
            try {
                List<String> waveElements = waveToData.get(levelAndWave).spriteNames.stream().map(ImageView::getId).collect(Collectors.toList());
                waveElements.addAll(elementsToSpawn);
                waveProperties.put("templatesToFire", waveElements);
                myController.editWaveProperties(waveToData.get(levelAndWave).waveId - 1,
            ...
            tempArray.addAll(waveToData.get(levelAndWave).spriteNames);
            waveToData.put(levelAndWave, new Data(tempArray, waveToData.get(levelAndWave).waveId));
        } else {
            //try {
